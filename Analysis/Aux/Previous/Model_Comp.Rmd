...
models <- c("Simple", "Unconditional Mean", "ARCH-M", "ApARCH", "GJR ARCH", "eARCH", "t-dist Simple", "t-dist apARCH", "t-dist GJR ARCH", "t-dist eARCH")


base.results <- data.frame(models)
arma.results <- data.frame(models)
...
Again I determine to not include ARCH in mean or the unconditional mean in this model specification as it does not improve model fit or produce significant coefficients. A Student-t distribution fits better than a normal distribution. Asymmetric models are not favorable for normal distributions but *are* preferred under a student-t distributions. Fitting an (3,1) ARMA process improves model fit significant. The best fitting model is an Asymmetric Power ARCH with a student t distribution and an ARCH in mean. 

**UGA Summary**

UGA results are very similiar to USO both with and without an ARMA process. Asymmetric models performed better, student t distributions have a better fit, and even ARCH-M produces significant coefficients *but* does not produce a better fit as in USO. The best fit is has a (2,1) ARMA process, is a student t distribution, and is an Exponential Power GARCH. 
...
corn.aic <- c(infocriteria(base.1.fit)[1])
...
corn.aic <- append(corn.aic, infocriteria(base.1.fit)[1])
...
corn.arma.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
corn.arma.aic <- append(corn.arma.aic, infocriteria(base.1.fit)[1])
```
...
arma.results$'CORN(0,1)' <- corn.arma.aic
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
soyb.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
soyb.aic <- append(soyb.aic, infocriteria(base.1.fit)[1])
```
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
soyb.arma.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
soyb.arma.aic <- append(soyb.arma.aic, infocriteria(base.1.fit)[1])
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
soyb.arma.aic <- append(soyb.arma.aic, NA)
```
```{r soyb_mute, echo = TRUE}
...
base.results$SOYB <- soyb.aic
arma.results$'SOYB(2,3)' <- soyb.arma.aic
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
weat.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
weat.aic <- append(weat.aic, infocriteria(base.1.fit)[1])
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
weat.arma.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
weat.arma.aic <- append(weat.arma.aic, infocriteria(base.1.fit)[1])
...
base.results$WEAT <- weat.aic
arma.results$'WEAT(1,5)' <- weat.arma.aic
```
...
```{r uso_setup, echo = FALSE}
x <- USO
```

Lets begin by looking at the OLS results. We fail to reject $\alpha = 0$ but reject the second hypothesis of $\beta = 1$ by quite a bit. The $\beta$ estimate of 0.9 implies that severe disunity.

```{r uso_ols, echo = FALSE}
ols <- lm(x$per_ETF_return ~ x$per_asset_return)
summary(ols)
x$etf_asset_error <- ols$residuals
```
...
uso.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
uso.aic <- append(uso.aic, infocriteria(base.1.fit)[1])
...
I find not reason to move to an asymmetric model.
...
Again, I will replace the normal distribution with the student t distribution. Model fit improves and now the asymmetric models fit better. 
...
(3,2) is the best model specification


```{r uso_arma1, echo=FALSE}
checkresiduals(x$etf_asset_error)
...
Now let's start over from the beginning with out base GARCH model and see if we should include the unconditional mean. Mu is not significant and does not improve model fit. 

```{r uso_arma2, echo=FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1)),
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
uso.arma.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
uso.arma.aic <- append(uso.arma.aic, infocriteria(base.1.fit)[1])

```

Including the ARCHM process actually produces a significant coefficient and improves fit! I will come back to this, but for now I leave it out. 

```{r uso_arma3, echo = FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1)),
                          mean.model = list(armaOrder = c(3,2), include.mean = FALSE))
base.1.fit <- ugarchfit(data = x$etf_asset_error, spec = base.1.spec, solver = 'hybrid')
...
Now we fit asymmetric models. Just like the models with an ARMA process, an Asymmetric model has a better fit. The very best model fit based off AIC is Asymmetric Power ARCH (same as WEAT I think)


```{r uso_arma5, echo=FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1)),
                          mean.model = list(armaOrder = c(3,2), include.mean = FALSE),
                          distribution.model = 'std')
base.1.fit <- ugarchfit(data = x$etf_asset_error, spec = base.1.spec, solver = 'hybrid')
...
Now I want to throw back in ARCH in mean, as we saw that improve model fit. 

```{r uso_final, echo = FALSE}
...
"Asymmetric Power GARCH (apGARCH) + ARCHM Robust Coefficients"
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)

base.results$USO <- uso.aic
arma.results$'USO(3,1)' <- uso.arma.aic
```
...
```{r uga_setup, echo = FALSE}
x <- UGA
```

Lets begin by looking at the OLS results. We fail to reject the null hypothesis of $\alpha = 0$ but reject the hypothesis $\beta = 1$. Just like USO, the $\beta$ coefficient is significantly below 1, inidcating severe disunity. 

```{r uga_ols, echo = FALSE}
ols <- lm(x$per_ETF_return ~ x$per_asset_return)
summary(ols)
x$etf_asset_error <- ols$residuals
```
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
uga.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
uga.aic <- append(uga.aic, infocriteria(base.1.fit)[1])
```
...
No reason to move to an asymmetric model.
...
Again, I will replace the normal distribution with the student t distribution. Model fit improves extremely slightly and suddenly the asymmetric models have a better fit. I will come back to this. 
...
(2,1) is the best model specification


```{r uga_arma1, echo=FALSE}
checkresiduals(x$etf_asset_error)
...
Now let's start over from the beginning with out base GARCH model and see if we should include the unconditional mean. It does not improve fit or produce significant estimates. I will leave it out. 

```{r uga_arma2, echo=FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1)),
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
uga.arma.aic <- infocriteria(base.1.fit)[1]
...
base.1.fit@fit[['robust.matcoef']]
infocriteria(base.1.fit)
uga.arma.aic <- append(uga.arma.aic, infocriteria(base.1.fit)[1])

```

Including the ARCHM process does not produce a better fit but does produce a singificant estimate. I will come back. 

```{r uga_arma3, echo = FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1)),
                          mean.model = list(armaOrder = c(2,1), include.mean = FALSE))
base.1.fit <- ugarchfit(data = x$etf_asset_error, spec = base.1.spec, solver = 'hybrid')
...
Now lets compare normal distribution with student-t. Again we find student-t distribution to have a better fit. 
```{r uga_arma4, echo=FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1)),
                          mean.model = list(armaOrder = c(2,1), include.mean = FALSE),
                          distribution.model = 'norm')
base.1.fit <- ugarchfit(data = x$etf_asset_error, spec = base.1.spec, solver = 'hybrid')
...
Now we fit asymmetric models and find that just like the non ARMA models, asymmetrics have a better fit. 


```{r uga_arma5, echo=FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1)),
                          mean.model = list(armaOrder = c(2,1), include.mean = FALSE),
                          distribution.model = 'std')
base.1.fit <- ugarchfit(data = x$etf_asset_error, spec = base.1.spec, solver = 'hybrid')
...
As Exponential GARCH has the best fit, I will try adding back in ARCHM like I did with USO. It does not produce a better fit. 

```{r uga_final, echo = FALSE}
base.1.spec <- ugarchspec(variance.model = list(garchOrder = c(1,1), model = "eGARCH"),
                          mean.model = list(armaOrder = c(2,1), include.mean = FALSE,
                                            archm=TRUE),
                          distribution.model = 'std')
...
base.results$UGA <- uga.aic
arma.results$'UGA(2,1)' <- uga.arma.aic
...
```{r echo = FALSE}
library(knitr)
kable(base.results, caption = "Model Results (no ARMA Process)")
kable(arma.results, caption = "Model Results (ARMA Process)")
